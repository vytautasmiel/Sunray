...
bool UBLOX::executeConfigGroup(int groupIndex, int timeout)
{
  bool ok = true; 
  switch (groupIndex)
  {
  case 0:                                                        // Enabled Ports
    ok &= configGPS.newCfgValset8(0x10530005, 1, VAL_LAYER_RAM); // CFG-UART2-ENABLED (on)
    //ok &= configGPS.addCfgValset8(0x10520005, 1);              // CFG-UART1-ENABLED (on)
    //ok &= configGPS.addCfgValset8(0x10510003, 0);              // CFG-I2C-ENABLED (off)
    ok &= configGPS.sendCfgValset8(0x10650001, 1, timeout*2);      // CFG-USB-ENABLED (on)
    break;

  case 1:                           
  // Disable USB Output Messages (most)
    ok &= configGPS.newCfgValset8(0x20910009, 0, VAL_LAYER_RAM); // CFG-MSGOUT-UBX_NAV_PVT_USB    (off)
    ok &= configGPS.addCfgValset8(0x209100bd, 0);                // CFG-MSGOUT-NMEA_ID_GGA_USB   (off)
    ok &= configGPS.addCfgValset8(0x209100c7, 0);                // CFG-MSGOUT-NMEA_ID_GSV_USB   (off)
    ok &= configGPS.addCfgValset8(0x209100cc, 0);                // CFG-MSGOUT-NMEA_ID_GLL_USB   (off)
    ok &= configGPS.addCfgValset8(0x209100b3, 0);                // CFG-MSGOUT-NMEA_ID_VTG_USB   (off)
    ok &= configGPS.addCfgValset8(0x209100c2, 0);                // CFG-MSGOUT-NMEA_ID_GSA_USB   (off)
    ok &= configGPS.addCfgValset8(0x20910090, 0);                // CFG-MSGOUT-UBX_NAV_RELPOSNED_USB  (off)
    ok &= configGPS.addCfgValset8(0x20910036, 0);                // CFG-MSGOUT-UBX_NAV_HPPOSLLH_USB   (off)
    ok &= configGPS.addCfgValset8(0x20910045, 0);                // CFG-MSGOUT-UBX_NAV_VELNED_USB     (off)
    ok &= configGPS.addCfgValset8(0x2091026b, 0);                // CFG-MSGOUT-UBX_RXM_RTCM_USB   (off)
    ok &= configGPS.addCfgValset8(0x20910348, 0);                // CFG-MSGOUT-UBX_NAV_SIG_USB   (off)
    ok &= configGPS.addCfgValset8(0x2091005e, 0);                // CFG-MSGOUT-UBX_NAV_TIMEUTC_USB   (off)
    ok &= configGPS.addCfgValset8(0x20910352, 0);                // CFG-MSGOUT-UBX-MON-COMMS_USB   (off)
    ok &= configGPS.sendCfgValset8(0x209100ae, 0, timeout);      // CFG-MSGOUT-NMEA_ID_RMC_USB   (off)

    break;

  case 2:                                                        // Disable UART1 Output Messages
    ok &= configGPS.newCfgValset8(0x20910007, 0, VAL_LAYER_RAM); // CFG-MSGOUT-UBX_NAV_PVT_UART1   (off)
    ok &= configGPS.addCfgValset8(0x2091008e, 0);                // CFG-MSGOUT-UBX_NAV_RELPOSNED_UART1  (off)
    ok &= configGPS.addCfgValset8(0x20910034, 0);                // CFG-MSGOUT-UBX_NAV_HPPOSLLH_UART1   (off)
    ok &= configGPS.addCfgValset8(0x20910043, 0);                // CFG-MSGOUT-UBX_NAV_VELNED_UART1     (off)
    ok &= configGPS.addCfgValset8(0x20910269, 0);                // CFG-MSGOUT-UBX_RXM_RTCM_UART1   (off)
    ok &= configGPS.addCfgValset8(0x20910346, 0);                // CFG-MSGOUT-UBX_NAV_SIG_UART1   (off)
    ok &= configGPS.sendCfgValset8(0x2091005c, 0, timeout);      // CFG-MSGOUT-UBX_NAV_TIMEUTC_UART1   (off)
    break;

  case 3:                                                        // Configure UART2 Output Messages (NMEA for external use, e.g., Radio/NTRIP)
    ok &= configGPS.newCfgValset8(0x209100a8, 0, VAL_LAYER_RAM); // CFG-MSGOUT-NMEA_ID_DTM_UART2  (off)
    ok &= configGPS.addCfgValset8(0x209100df, 0);                // CFG-MSGOUT-NMEA_ID_GBS_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100bc, 60);               // CFG-MSGOUT-NMEA_ID_GGA_UART2  (every 60 solutions)
    ok &= configGPS.addCfgValset8(0x209100cb, 0);                // CFG-MSGOUT-NMEA_ID_GLL_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100b7, 0);                // CFG-MSGOUT-NMEA_ID_GNS_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100d0, 0);                // CFG-MSGOUT-NMEA_ID_GRS_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100c1, 0);                // CFG-MSGOUT-NMEA_ID_GSA_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100d5, 0);                // CFG-MSGOUT-NMEA_ID_GST_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100c6, 0);                // CFG-MSGOUT-NMEA_ID_GSV_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100ad, 0);                // CFG-MSGOUT-NMEA_ID_RMC_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100e9, 0);                // CFG-MSGOUT-NMEA_ID_VLW_UART2   (off)
    ok &= configGPS.addCfgValset8(0x209100b2, 0);                // CFG-MSGOUT-NMEA_ID_VTG_UART2   (off)
    ok &= configGPS.sendCfgValset8(0x209100da, 0, timeout);      // CFG-MSGOUT-NMEA_ID_ZDA_UART2  (off)
    break;

  case 4: // Configure UART2 Protocols and Baud Rate (for Radio/NTRIP)
    // UART2 Protocols (Xbee/NTRIP)
    ok &= configGPS.newCfgValset8(0x10750001, 0, VAL_LAYER_RAM); // CFG-UART2INPROT-UBX        (off)
    ok &= configGPS.addCfgValset8(0x10750002, 0);                // CFG-UART2INPROT-NMEA       (off)
    ok &= configGPS.addCfgValset8(0x10750004, 1);                // CFG-UART2INPROT-RTCM3X     (on)
    ok &= configGPS.addCfgValset8(0x10760001, 0);                // CFG-UART2OUTPROT-UBX       (off)
    ok &= configGPS.addCfgValset8(0x10760002, 1);                // CFG-UART2OUTPROT-NMEA      (on)
    ok &= configGPS.addCfgValset8(0x10760004, 0);                // CFG-UART2OUTPROT-RTCM3X    (off)
    // UART2 Baud Rate (Xbee/NTRIP)
    ok &= configGPS.sendCfgValset32(0x40530001, 115200, timeout); // CFG-UART2-BAUDRATE
    break;

  case 5:                                                        // Configure UART1 Protocols (Ardumower)
    ok &= configGPS.newCfgValset8(0x10730001, 1, VAL_LAYER_RAM); // CFG-UART1INPROT-UBX     (on)
    ok &= configGPS.addCfgValset8(0x10730002, 0);                // CFG-UART1INPROT-NMEA    (off)
    ok &= configGPS.addCfgValset8(0x10730004, 0);                // CFG-UART1INPROT-RTCM3X  (off)
    ok &= configGPS.addCfgValset8(0x10740001, 1);                // CFG-UART1OUTPROT-UBX    (on)
    ok &= configGPS.addCfgValset8(0x10740002, 0);                // CFG-UART1OUTPROT-NMEA   (off)
    ok &= configGPS.sendCfgValset8(0x10740004, 0, timeout);      // CFG-UART1OUTPROT-RTCM3X (off)
    break;

  case 6:                                                        // Configure USB Protocols (Ardumower)
    ok &= configGPS.newCfgValset8(0x10770001, 1, VAL_LAYER_RAM); // CFG-USBINPROT-UBX     (on)
    ok &= configGPS.addCfgValset8(0x10770002, 1);                // CFG-USBINPROT-NMEA    (on)
    ok &= configGPS.addCfgValset8(0x10770004, 0);                // CFG-USBINPROT-RTCM3X  (off)
    ok &= configGPS.addCfgValset8(0x10780001, 1);                // CFG-USBOUTPROT-UBX    (on)
    ok &= configGPS.addCfgValset8(0x10780002, 0);                // CFG-USBOUTPROT-NMEA   (off)
    ok &= configGPS.sendCfgValset8(0x10780004, 0, timeout);      // CFG-USBOUTPROT-RTCM3X (off)
    break;

  case 7: // Navigation Settings (Fix Mode, Altitude Constraints, Filters, Rates)
    // Fix Mode & Altitude Constraints
    ok &= configGPS.newCfgValset8(0x20110011, 3, VAL_LAYER_RAM); // CFG-NAVSPG-FIXMODE    (auto)
    ok &= configGPS.addCfgValset8(0x10110013, 0);                // CFG-NAVSPG-INIFIX3D   (no initial 3D fix required)
    ok &= configGPS.addCfgValset32(0x401100c1, 10000);           // CFG-NAVSPG-CONSTR_ALT (100m altitude constraint)
    // Input Filters (Use custom or default based on GPS_CONFIG_FILTER)
    if (GPS_CONFIG_FILTER)
    {                                                                        // custom filter settings
      ok &= configGPS.addCfgValset8(0x201100a4, CPG_CONFIG_FILTER_MINELEV);  // CFG-NAVSPG-INFIL_MINELEV
      ok &= configGPS.addCfgValset8(0x201100aa, CPG_CONFIG_FILTER_NCNOTHRS); // CFG-NAVSPG-INFIL_NCNOTHRS
      ok &= configGPS.addCfgValset8(0x201100ab, CPG_CONFIG_FILTER_CNOTHRS);  // CFG-NAVSPG-INFIL_CNOTHRS
    }
    else
    {                                                // ublox default filter settings
      ok &= configGPS.addCfgValset8(0x201100a4, 10); // CFG-NAVSPG-INFIL_MINELEV (10 deg)
      ok &= configGPS.addCfgValset8(0x201100aa, 0);  // CFG-NAVSPG-INFIL_NCNOTHRS (0)
      ok &= configGPS.addCfgValset8(0x201100ab, 0);  // CFG-NAVSPG-INFIL_CNOTHRS (0 dBHz)
    }
    ok &= configGPS.addCfgValset8(0x201100c4, GPS_CONFIG_DGNSS_TIMEOUT); // CFG-NAVSPG-CONSTR_DGNSSTO (DGNSS timeout)
    // Measurement and Navigation Rates (5 Hz)
    ok &= configGPS.addCfgValset16(0x30210001, 200);         // CFG-RATE-MEAS (200 ms measurement period)
    ok &= configGPS.sendCfgValset16(0x30210002, 1, timeout); // CFG-RATE-NAV (1 navigation cycle per measurement)
    break;

  case 8:                                                        // Enable Required USB Output Messages (Ardumower)
    ok &= configGPS.newCfgValset8(0x20910009, 0, VAL_LAYER_RAM); // CFG-MSGOUT-UBX_NAV_PVT_USB (off)
    ok &= configGPS.addCfgValset8(0x20910090, 1);                // CFG-MSGOUT-UBX_NAV_RELPOSNED_USB (every solution)
    ok &= configGPS.addCfgValset8(0x20910036, 1);                // CFG-MSGOUT-UBX_NAV_HPPOSLLH_USB  (every solution)
    ok &= configGPS.addCfgValset8(0x20910045, 1);                // CFG-MSGOUT-UBX_NAV_VELNED_USB    (every solution)
    ok &= configGPS.addCfgValset8(0x2091026b, 5);                // CFG-MSGOUT-UBX_RXM_RTCM_USB    (every 5 solutions)
    ok &= configGPS.addCfgValset8(0x20910348, 20);               // CFG-MSGOUT-UBX_NAV_SIG_USB     (every 20 solutions)
    ok &= configGPS.addCfgValset8(0x2091005e, 0);                // CFG-MSGOUT-UBX_NAV_TIMEUTC_USB (off)
    ok &= configGPS.addCfgValset8(0x209100bd, 60);               // CFG-MSGOUT-NMEA_ID_GGA_USB     (every 60 solutions)
    ok &= configGPS.sendCfgValset8(0x20910352, 70, timeout);     // CFG-MSGOUT-UBX-MON-COMMS_USB (every 70 solutions)
    break;

  case 9:                                                        // Enable Required UART1 Output Messages (Ardumower)
    ok &= configGPS.newCfgValset8(0x20910007, 0, VAL_LAYER_RAM); // CFG-MSGOUT-UBX_NAV_PVT_UART1 (off)
    ok &= configGPS.addCfgValset8(0x2091008e, 1);                // CFG-MSGOUT-UBX_NAV_RELPOSNED_UART1 (every solution)
    ok &= configGPS.addCfgValset8(0x20910034, 1);                // CFG-MSGOUT-UBX_NAV_HPPOSLLH_UART1  (every solution)
    ok &= configGPS.addCfgValset8(0x20910043, 1);                // CFG-MSGOUT-UBX_NAV_VELNED_UART1    (every solution)
    ok &= configGPS.addCfgValset8(0x20910269, 5);                // CFG-MSGOUT-UBX_RXM_RTCM_UART1    (every 5 solutions)
    ok &= configGPS.addCfgValset8(0x20910346, 20);               // CFG-MSGOUT-UBX_NAV_SIG_UART1     (every 20 solutions)
    ok &= configGPS.sendCfgValset8(0x2091005c, 0, timeout);      // CFG-MSGOUT-UBX_NAV_TIMEUTC_UART1 (off)
    break;

  default:
    // Should not happen if called with index 0-9
    ok = false;
    break;
  }
  // Assuming the SparkFun library functions return false on failure/timeout
  return ok;
}

// --- Helper Function to try executing a config group with one retry ---
bool UBLOX::tryExecuteConfigGroupWithRetry(int groupIndex, int timeout)
{
  bool success = false;
  int maxAttempts = 10;
  for (int attempt = 1; attempt <= maxAttempts; attempt++)
  {
    CONSOLE.print("..."); // Indicate attempt start
    if (executeConfigGroup(groupIndex, timeout))
    {
      success = true;
    ...
    else
    {
      ...
      {
        CONSOLE.print(" Retrying..."); // Indicate retry
...
bool UBLOX::configure()
{
  ...
  configGPS.enableDebugging(CONSOLE, true); // Uncomment if needed
  ...
  while (true)
  {
    ...
    {
      connected = true;
      break; // Successfully connected at target baud rate
    }

    CONSOLE.println(F("ERROR: GPS receiver is not responding at target baud rate."));
    CONSOLE.println("Trying baud 38400 as fallback...");
    _bus->begin(38400); // Try fallback baud rate
    if (configGPS.begin(*_bus))
    {
      CONSOLE.println("GPS found at 38400. Setting configured baud rate...");
      // Attempt to set the desired baud rate and save to RAM layer
      configGPS.setVal32(0x40520001, _baud, VAL_LAYER_RAM); // CFG-UART1-BAUDRATE
      delay(100);                                           // Give module time to process
      _bus->begin(_baud);                                   // Re-initialize serial at the new target baud rate
      delay(100);
      // Need to verify connection again at the new baud rate
      if (configGPS.begin(*_bus))
      {
        CONSOLE.println("Successfully set target baud rate.");
        connected = true;
        break;
      }
      else
      {
        CONSOLE.println("ERROR: Failed to re-establish connection at target baud rate after setting it.");
        // Potentially revert baud rate setting or handle error
      }
    }
    _bus->begin(_baud); // Revert to trying original baud if fallback failed

    CONSOLE.println(F("ERROR: GPS receiver is not responding at any tested baud rate."));
    Logger.event(EVT_ERROR_GPS_NOT_CONNECTED);
    // Consider adding a delay or limiting retries here if desired
    // For now, it continues trying indefinitely as per original logic
    delay(1000); // Add a small delay before retrying connection loop
  }

  if (!connected)
  {
    // Should generally not happen with the while(true) loop unless modified to exit
    CONSOLE.println("ERROR: Exited connection loop without success.");
    return false;
  }

  CONSOLE.println("GPS receiver found and connected!");

  // --- Get Protocol Version ---
  if (configGPS.getProtocolVersion())
  { // Check if version retrieval succeeded
  ...
  {
    CONSOLE.println("Warning: Could not retrieve UBLOX protocol version.");
  }
  ...
  int timeout = 2000; // Standard timeout for config messages
  int successfulTasks = 0;
  const int totalTasks = 10; // Total number of configuration groups

  // --- Execute Configuration Tasks ---
  for (int idx = 0; idx < totalTasks; ++idx)
  {
    CONSOLE.print("Config Group ");
    CONSOLE.print(idx);
    if (tryExecuteConfigGroupWithRetry(idx, timeout))
    {
      CONSOLE.println(" OK");
      successfulTasks++;
    }
    else
    {
      ...
      // Optionally: break here if any single task failure is critical,
      // e.g., if (!ok) { /* handle critical failure */ break; }
    }
    watchdogReset(); // Reset watchdog AFTER successful group configuration
  }
  ...
  {
    CONSOLE.print("ERROR: config sending failed. Succeeded tasks: ");
    CONSOLE.print(successfulTasks);
    CONSOLE.print("/");
    CONSOLE.println(totalTasks);
    return false;
  }
}

// bool UBLOX::configure1(){
//   CONSOLE.println("trying to connect to ublox f9p...");
...
//       if (idx == 0){
//         // ----- enabled ports -----------------
//         setValueSuccess &= configGPS.newCfgValset8(0x10530005, 1, VAL_LAYER_RAM); // CFG-UART2-ENABLED  (off/on)
//         setValueSuccess &= configGPS.addCfgValset8(0x10520005, 1); // CFG-UART1-ENABLED (off/on)
...
//         setValueSuccess &= configGPS.addCfgValset8(0x10770004, 0); // CFG-USBINPROT-RTCM3X  (on/off)
//         setValueSuccess &= configGPS.addCfgValset8(0x10780001, 1); // CFG-USBOUTPROT-UBX    (on)
//         setValueSuccess &= configGPS.addCfgValset8(0x10780002, 0); // CFG-USBOUTPROT-NMEA   (on/off)
  ...
  case 0x01:
    ...
    { // UBX-NAV-SIG
      ...
      for (int i = 0; i < numSigs; i++)
      {
        ...
        { // signal is healthy
          if (prUsed)
          { // pseudorange has been used (indicates satellites will be also used for carrier correction)
            // if (cno > 0){  // signal has some strength (carriar-to-noise)
            healthycnt++;
            if (crCorrUsed)
            { // Carrier range corrections have been used
            ...
            for (int j = 0; j < 4; j++)
            {
              ...
              if ((protId != 0xFF) && (msgs != 0))
              {
                ...
                case 5:
                  ...
                  break;
                case 6:
                  CONSOLE.print(" SPARTN=");
                  break;
                default:
                  CONSOLE.print(" ");
                  CONSOLE.print(protId, HEX);
                  CONSOLE.print("=");
                }
                CONSOLE.print(msgs);
              }
